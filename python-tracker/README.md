...
The Unity background feed is a clean plate: tracker landmarks are never drawn into it, and a missed detection holds the previous clean frame instead of exposing a raw camera frame. Add `--show-landmarks` only for a local debug preview.
