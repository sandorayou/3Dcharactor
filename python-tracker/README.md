...
Run a camera with `python app.py`, or a file with `python app.py --source path\\to\\video.mp4`. The raw camera feed and the landmark tracker preview open in separate windows. Use `--no-preview` to disable both windows for the lowest overhead. The capture thread overwrites its one-frame buffer, so inference never works through stale frames.
