...
配信中にアプリを離れる、バックグラウンドへ移る、またはオブジェクトが破棄される場合は、自動的に配信を停止します。停止ボタンは配信画面上に常時配置してください。Twitch/YouTubeの「アカウント連携」ボタンは`LiveStreamAccountLinkController`からOAuth開始URLを開けますが、認証コードをサーバーで交換して配信キーを取得する処理は別途必要です。クライアントシークレットをUnityへ入れないでください。
